...
We will be using the data from last week. You should have already downloaded all this data for last week's practical lesson - if you have not done so already you can access them by [**clicking here**](https://github.com/UCLPG-MSC-SGDS/GEOG0114/raw/main/Week%207%20-%20Dataset.zip). 
...
The object `lsoa_result` is a non-spatial data frame containing our results. We need to merge them back into the shapefile.

```{r eval = FALSE}
lsoa_result_gwr <- merge(lsoa.shp, lsoa_result, by.x = "LSOACODE", by.y = "LSOACODE")
```
...
```{r eval = FALSE}
summary(lsoa_result_gwr$CoefLogIMD)
```
