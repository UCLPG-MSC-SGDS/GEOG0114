---
title: "Spatial analysis for data sciences"
---
...
Let us install following packages: `remotes`, `tmap` (version 3.3) and `sf` using the `install.packages()` and `install_version()` functions, and then activate the packages to make them active using the `library()` function. 
...
:::{.callout-note}
If prompted with the following message: `Which would you like to update?` - type `1` for **"All"** and press the enter button `↵` to continue the installation for `tmap` - this will ensure all updated versions of other packages related to `tmap` are installed.
:::
