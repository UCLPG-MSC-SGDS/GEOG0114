...
- For **Windows** or **macOS (Intel)** users - please download the latest version of Java [**[HERE]**](https://www.java.com/en/download/)
- For **macOS M1/M2** users - please download the latest version of **Azul Zulu Builds of OpenJDK** [**[HERE]**](https://www.azul.com/downloads/?package=jdk#zulu). Note that this is an open source program that supports Java and allows Java to operate smoothly on a **macOS M1/M2** system.
</div>
...
<div class="note">
**IMPORTANT NOTES**: For **macOS M1/M2** users - please use the `Sys.setenv()` function to configure your macOS system to that Java home path accordingly to prevent RStudio from crashing with that annoying **Abort** message whenever the `maxent()` function is executed.
</div>
...
<div class="note">
**IMPORTANT NOTES**: All shape file and raster data (5km resolution) were projected to the CRS: WGS84 4326.
</div>
...
tm_shape(california_county) + 
	tm_fill("AtRiskCat", style = "cat", title = "Popn at risk [%]", palette = c("white", "#ffffe5", "#fed976", "#fc4e2a", "#bd0026"), labels = Risk_tier) +
	tm_borders(col = "black") + tm_text("NAME", size = "AREA") +
	tm_scale_bar(position=c(0.02, 0.02), text.size = 1, breaks = c(0, 100, 200, 300)) +
	tm_compass(north = 0,type = 'arrow', position = c('right', 'top'), text.size = 0.9) +
	tm_layout(frame = FALSE, legend.outside = TRUE)
```
...
- Use the following `set.seed(set.seed(20000430)`.
